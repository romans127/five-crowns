...
import { GinMark, GinRace } from './Brand.tsx'
import {
  canCallGame,
  knockerPlayer,
  playerColor,
...
import { leftoverHint, scoreLegend, winConditionCopy } from './rules.ts'
import {
  canResume,
  deleteHistoryGame,
  formatWhen,
...
import { MAX_PLAYERS, MIN_PLAYERS, PLAYER_COLORS, type GinGame, type GinRecord } from './types.ts'

export function GinHome({
  canResume: resume,
  historyCount,
  onLeaveGames,
  onNewGame,
  ...
  return (
    <SceneShell
      ...
      flowLinks={[
        ...
        { label: 'Gin board', onClick: onLeaderboard },
        { label: 'Rules', onClick: onRules },
      ]}
    >
      <div className="hero-block">
        <GinMark />
        <p className="eyebrow">Knock or gin</p>
        <h2 className="hero-display">Gin Rummy</h2>
        <p className="tagline">Count leftover deadwood. Gin bonus, undercut, and a race to 100.</p>
      </div>
...
export function GinSetup({ onBack, onStart }: { onBack: () => void; onStart: (names: string[]) => void }) {
  ...
  return (
    <SceneShell
      ...
      nav={{
        back: { label: 'Gin Rummy', onClick: onBack },
        title: 'New table',
        subtitle: 'Who’s knocking?',
      }}
    >
      <p className="hint">Two to six players. First to 100 from leftover deadwood, gin, and undercuts.</p>
...
export function GinPlay({
  ...
  game: GinGame
  onBack: () => void
  onScore: (playerId: string, leftover: number | null, knocker: boolean, gin: boolean) => void
  onNextRound: () => void
  onCallGame: () => void
  onRules: () => void
  onQuit: () => void
  ...
  const knocker = knockerPlayer(game)
  ...
  return (
    <SceneShell
      ...
      nav={{
        ...
        subtitle: `First to ${game.winThreshold}`,
        actions: [{ label: 'Rules', onClick: onRules }],
      }}
    >
      <h1 className="play-title">Count the deadwood</h1>
      <GinRace
        ...
        {game.players.map((player, index) => {
          ...
          return (
            <PlayerScoreRow
              ...
              scoreLabel={score?.gin ? 'Gin' : score?.knocker ? 'Knock' : entered ? score?.leftover ?? 'Tap' : 'Tap'}
              entered={entered}
      ...
      {ready ? (
        ...
        <p className="hint center frost-tile">
          {knocker
            ? `${knocker.name} ${current[knocker.id]?.gin ? 'went gin' : 'knocked'}. Enter leftover deadwood.`
            : 'Tap a player to mark the knocker, then enter leftover deadwood.'}
        </p>
      )}
      ...
      {editing ? (
        <GinPad
          ...
          currentKnocker={current[editing.id]?.knocker ?? false}
          currentGin={current[editing.id]?.gin ?? false}
          onClose={() => setEditingId(null)}
          onSave={(leftover, knockerFlag, gin) => {
            onScore(editing.id, leftover, knockerFlag, gin)
            setEditingId(null)
          }}
        />
      ) : null}
...
function GinPad({
  ...
  currentKnocker: boolean
  currentGin: boolean
  onSave: (leftover: number, knocker: boolean, gin: boolean) => void
  onClose: () => void
}) {
  const [digits, setDigits] = useState(currentLeftover === null || currentGin ? '' : String(currentLeftover))
  const [knocker, setKnocker] = useState(currentKnocker)
  const [gin, setGin] = useState(currentGin)
  const total = gin ? 0 : digits === '' ? 0 : Number(digits)
    ...
    >
      <div className="score-pad-body">
        ...
        <p className="pad-total" aria-live="polite">
          {gin ? '0' : digits === '' ? '0' : digits}
          <span>{gin ? 'gin' : knocker ? 'knocker leftover' : 'leftover deadwood'}</span>
        </p>
          ...
          <div className="keypad" aria-label="Score keypad">
            {['1', '2', '3', '4', '5', '6', '7', '8', '9'].map((key) => (
              <Button
                ...
                onClick={() => setDigits((current) => (current + key).replace(/^0+(?=\d)/, '').slice(0, 2))}
              >
                {key}
              </Button>
            ))}
            ...
            <Button
              ...
              size="large"
              className="keypad-digit"
              onClick={() => setDigits((current) => (current + '0').replace(/^0+(?=\d)/, '').slice(0, 2))}
            >
              0
            </Button>
            <Button
      ...
      <div className="sheet-actions">
        <TintedButton onClick={() => onSave(0, true, true)}>Gin · 0</TintedButton>
        <PrimaryButton onClick={() => onSave(total, knocker, gin)}>Lock in {total}</PrimaryButton>
      </div>
    </Sheet>
  )
}

export function GinRules({ onBack, backLabel = 'Table' }: { onBack: () => void; backLabel?: string }) {
  return (
    <SceneShell
      ...
      nav={{
        ...
        title: 'Rules',
        subtitle: 'How gin works',
      }}
    >
      <h2 className="scene-section-title">How Gin Rummy works</h2>
      <article className="rule-card">
        <h2>Melds and leftover</h2>
        <GinMark />
        <p>Sets and runs stay down. Unmatched cards are leftover deadwood. Knock when you think yours is lower.</p>
      </article>
      <article className="rule-card">
        ...
        <p>With more than two players, the knocker is scored against each opponent separately.</p>
      </article>
      <article className="rule-card">
        <h2>Winning</h2>
...
export function GinWinner({
  ...
  return (
    ...
    >
      <div className="hero-block">
        <GinMark />
        <p className="eyebrow">{reached ? 'Hit 100' : 'Table called'}</p>
        <h1>{champs.length > 1 ? 'Shared win!' : 'Gin!'}</h1>
      </div>
      <p className="champs">
        {champs.map((player) => (
...
export function GinHistory({
  ...
  onBack: () => void
  onLeaderboard: () => void
  onResume: (record: GinRecord) => void
}) {
    ...
    <SwipeBack onBack={onBack}>
      <SceneShell
        ...
        nav={{
          ...
          actions: [{ label: 'Gin board', onClick: onLeaderboard }],
        }}
        flowLinks={[{ label: 'Gin board', onClick: onLeaderboard }]}
      >
        <SearchBar value={query} onChange={setQuery} placeholder="Search players…" aria-label="Search past games" />
        {filtered.length === 0 ? (
          <p className="hint center history-empty">No saved gin games yet.</p>
        ) : (
...
export function GinLeaderboard({ onBack }: { onBack: () => void }) {
  ...
  return (
    ...
    >
      {entries.length === 0 ? (
        <p className="hint center history-empty">Finish a gin match and stats will show up here.</p>
      ) : (
        <>
