...
function archiveGame(game: CribbageGame, archived: Set<string>, force = false): void {
...
export function useCribbageGame() {
  const [game, setGame] = useState<CribbageGame | null>(() => loadGame())
  ...
  const stashCurrentGame = useCallback((current: CribbageGame | null) => {
    if (!current) {
      return
    }
