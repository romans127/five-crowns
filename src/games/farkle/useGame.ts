...
function archiveGame(game: FarkleGame, archived: Set<string>, force = false): void {
...
export function useFarkleGame() {
  const [game, setGame] = useState<FarkleGame | null>(() => loadGame())
  ...
  const stashCurrentGame = useCallback((current: FarkleGame | null) => {
    if (!current) {
      return
    }
  ...
  const recordScore = useCallback((playerId: string, points: number | null, farkle: boolean) => {
    setGame((current) => (current ? setRoundScore(current, playerId, points, farkle) : current))
  }, [])
