...
export const ACTIVE_KEY = 'sorry:v1'
export const HISTORY_KEY = 'sorry:history:v1'
export const MAX_HISTORY = 50

type ActiveStore = { version: 1; game: SorryGame | null }
type HistoryStore = { version: 1; records: SorryRecord[] }
...
export function loadGame(storage: Pick<Storage, 'getItem'> = localStorage): SorryGame | null {
  try {
...
export function saveGame(game: SorryGame | null, storage: Pick<Storage, 'setItem' | 'removeItem'> = localStorage): void {
  if (!game) {
...
export function listHistory(storage: Pick<Storage, 'getItem'> = localStorage): SorryRecord[] {
...
export function gameHasProgress(game: SorryGame): boolean {
  return game.players.some((player) => typeof game.scores[player.id]?.pawnsHome === 'number')
}

export function historyHeadline(record: SorryRecord): string {
  return record.players.map((player) => player.name).join(', ')
}

export function searchHistory(records: SorryRecord[], query: string): SorryRecord[] {
...
export function canResume(record: SorryRecord): boolean {
  return record.status === 'playing'
}

export function recordToGame(record: SorryRecord): SorryGame {
...
export async function hydrateFromCloud(): Promise<SorryRecord[]> {
  const rows = await listCloudRecords('sorry', 'history')
  return rows
    .map((row) => row.payload as SorryRecord)
