...
export const ACTIVE_KEY = 'hearts:v1'
export const HISTORY_KEY = 'hearts:history:v1'
export const MAX_HISTORY = 50

type ActiveStore = { version: 1; game: HeartsGame | null }
type HistoryStore = { version: 1; records: HeartsRecord[] }
...
export function loadGame(storage: Pick<Storage, 'getItem'> = localStorage): HeartsGame | null {
  try {
...
export function saveGame(game: HeartsGame | null, storage: Pick<Storage, 'setItem' | 'removeItem'> = localStorage): void {
  if (!game) {
...
export function listHistory(storage: Pick<Storage, 'getItem'> = localStorage): HeartsRecord[] {
...
export function gameHasProgress(game: HeartsGame): boolean {
  return game.rounds.some((round) => Object.values(round).some((score) => typeof score.points === 'number'))
}

export function handsRecorded(game: HeartsGame): number {
  return game.rounds.filter((_, index) => roundComplete(game, index)).length
}

export function historyHeadline(record: HeartsRecord): string {
  return record.players.map((player) => player.name).join(', ')
}

export function searchHistory(records: HeartsRecord[], query: string): HeartsRecord[] {
...
export function canResume(record: HeartsRecord): boolean {
  return record.status === 'playing'
}

export function recordToGame(record: HeartsRecord): HeartsGame {
...
export async function hydrateFromCloud(): Promise<HeartsRecord[]> {
  const rows = await listCloudRecords('hearts', 'history')
  return rows
    .map((row) => row.payload as HeartsRecord)
