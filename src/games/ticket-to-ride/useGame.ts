...
function archiveGame(game: TtrGame, archived: Set<string>, force = false): void {
...
export function useTtrGame() {
  const [game, setGame] = useState<TtrGame | null>(() => loadGame())
  ...
  const stashCurrentGame = useCallback((current: TtrGame | null) => {
    if (!current) {
      return
    }
  ...
  const recordScore = useCallback((playerId: string, routes: number | null, tickets: number | null, longest: number | null) => {
    setGame((current) => (current ? setPlayerScore(current, playerId, routes, tickets, longest) : current))
  }, [])
